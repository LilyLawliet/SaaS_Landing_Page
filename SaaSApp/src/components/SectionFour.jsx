...
import { Slack, Cloud, Cpu, Zap } from "lucide-react";

export default function SectionFour() {
  const integrations = [
    { name: "Slack", icon: <Slack size={42} className="text-indigo-400" /> },
    { name: "Cloud Storage", icon: <Cloud size={42} className="text-purple-400" /> },
    { name: "AI Engine", icon: <Cpu size={42} className="text-pink-400" /> },
    { name: "Zapier", icon: <Zap size={42} className="text-cyan-400" /> },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto text-center relative z-10">
        ...
        >
          Aurora AI Copilot integrates seamlessly with the tools you already use. 
          Zero disruption — insights delivered exactly where you need them.
        </motion.p>
      </div>

      {/* Integrations Grid with floating parallax */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-12 max-w-5xl mx-auto mt-20 relative z-10">
        {integrations.map((item, idx) => (
          <Parallax key={idx} translateY={[-10, 10]} speed={5}>
            ...
            >
              <div className="p-4 rounded-full bg-gradient-to-br from-indigo-500/20 to-purple-500/10 group-hover:scale-110 transition-transform">
                {item.icon}
              </div>
